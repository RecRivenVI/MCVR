...
void main() {
    ...
    float alpha = clamp(resolved.a * layer.a, 0.0, 1.0);
    ivec2 lightUv = ivec2(round(bary.x * vec2(m0.lightUV) + bary.y * vec2(m1.lightUV) + bary.z * vec2(m2.lightUV)));
    float light = hasLight(m0.packedData) ? clamp(float(max(lightUv.x, lightUv.y)) / 240.0, 0.0, 1.0) : 1.0;
    float emission = bary.x * m0.albedoEmission + bary.y * m1.albedoEmission + bary.z * m2.albedoEmission;
