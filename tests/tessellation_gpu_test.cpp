...
int main(int argc, char **argv) {
    ...
    try {
        ...
        const std::array<VkShaderStageFlagBits, 4> stageBits{VK_SHADER_STAGE_VERTEX_BIT,
        ...
        pipelineInfo.stageCount = 4; pipelineInfo.pStages = stages.data();
        pipelineInfo.pVertexInputState = &vertexInput; pipelineInfo.pInputAssemblyState = &assembly;
        pipelineInfo.pTessellationState = &tessellation; pipelineInfo.pViewportState = &viewportState;
            ...
            for (uint32_t x = 0; x < WIDTH; ++x) {
                ...
                for (uint32_t channel = 0; channel < 4; ++channel) {
                    if (!nearByte(pixels[base + channel], expected[channel])) {
                        throw std::runtime_error("Patch raster coverage/color mismatch at "
